...
`$.Annot` <-  function(x, name) {
  if (name %in% slotNames(x)) return(slot(x, name))
  x@annot[, name]
}
...
##' @param features character vector of pathway/feature names to keep in gsva slot
##' @param cells character vector of cell barcodes to keep
##' @param ... additional arguments passed to subset.Seurat (excluding features)
##' @export
subset.GSVA <-  function(x, features = NULL, cells = NULL, ...) {
 gsva <- x@gsva
 seu <- x@obj
 if (!is.null(features)) {
   valid_feat <- intersect(features, colnames(gsva))
   if (length(valid_feat) == 0) warning("None of the requested features found in gsva slot")
   gsva <- gsva[, valid_feat, drop = FALSE]
 }
 if (!is.null(cells)) {
   seu <- subset(seu, cells = cells, ...)
   gsva <- gsva[intersect(rownames(gsva), rownames(seu@meta.data)), , drop = FALSE]
 }
 return(new("GSVA", obj = seu, gsva = gsva, annot = x@annot))
}
...
.get_go_dat<-function(ont="BP"){
  if (!requireNamespace("GO.db", quietly = TRUE)) {
    stop("GO.db package is required. Install with BiocManager::install('GO.db')")
  }
  key <- keys(GO.db::GO.db)
  suppressMessages(go_dat <- AnnotationDbi::select(GO.db::GO.db, keys = key,
...
split_data_matrix <- function(matrix, chunk.size=1000) {
    ncols <- dim(matrix)[2]
