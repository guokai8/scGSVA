...
**scGSVA** is an R package for performing gene set enrichment analysis on single-cell RNA-seq data. It includes a **custom C++ optimized implementation** of four GSVA scoring methods (ssGSEA, GSVA, PLAGE, z-score) that produces identical results to the original GSVA Bioconductor package but runs up to 200x faster. A companion Python package [**pygsva**](https://github.com/guokai8/pygsva) provides the same methods for Python/Scanpy workflows.

### Key Features

- **Five enrichment methods**: ssGSEA, GSVA, PLAGE, z-score, and UCell
- **C++ optimized**: Built-in Rcpp implementation (up to 200x faster than original GSVA)
- **Parallel computing**: BiocParallel-based parallelization for large datasets
- **Flexible annotation building**: KEGG, GO, and MSigDB gene sets for 20+ species
- **Rich visualization**: Violin, dot, ridge, heatmap, feature, bar, lollipop, and spatial plots
...
- **Python companion**: [pygsva](https://github.com/guokai8/pygsva) for Python/Scanpy users
...
The main function `scgsva()` supports five enrichment methods:

| Method | Description | Reference |
|--------|-------------|-----------|
| `"ssgsea"` | Single-sample GSEA (default) | Barbie et al. 2009 |
| `"gsva"` | Gene Set Variation Analysis | Hänzelmann et al. 2013 |
| `"plage"` | Pathway Level Analysis of Gene Expression | Tomfohr et al. 2005 |
| `"zscore"` | Combined z-score | Lee et al. 2008 |
| `"UCell"` | UCell rank-based scoring | Andreatta & Carmona 2021 |

### 2.2 ssGSEA (Default)

```{r scgsva}
# Run ssGSEA (default method, uses fast C++ implementation)
...
```{r gsva_method}
# GSVA with Poisson kernel (for count data)
res_gsva <- scgsva(pbmcs, hsko, method = "gsva", kcdf = "Poisson")

# GSVA with Gaussian kernel (for continuous/normalized data)
# res_gsva <- scgsva(pbmcs, hsko, method = "gsva", kcdf = "Gaussian")
```

### 2.4 PLAGE and z-score Methods

```{r plage_zscore}
# PLAGE
res_plage <- scgsva(pbmcs, hsko, method = "plage")

# z-score
res_zscore <- scgsva(pbmcs, hsko, method = "zscore")
```

### 2.5 UCell Method

UCell is a rank-based method that's robust and fast. Gene signatures are automatically
filtered to genes present in the expression matrix, so `maxRank` is auto-adjusted
to prevent errors with large gene sets (e.g., KEGG pathways):

```{r ucell, eval=FALSE}
# Run UCell (requires UCell package)
res_ucell <- scgsva(pbmcs, hsko, method = "UCell")

# You can also specify a custom maxRank
res_ucell <- scgsva(pbmcs, hsko, method = "UCell", maxRank = 3000)
```

### 2.6 PLAID Method

PLAID (Pathway Level Average Intensity Detection) is an ultrafast gene set enrichment 
method using sparse matrix computation. It computes the average intensity of genes 
in each gene set for every sample. Reference: Zito et al., Bioinformatics 2025.

**Key advantages:**
- Very fast due to sparse matrix operations
- Memory efficient for large datasets
- Comparable accuracy to established methods

**Note:** PLAID works best with log-transformed expression data (e.g., Seurat 
normalized counts or log-transformed values).

```{r plaid}
# PLAID: Ultrafast sparse matrix method
res_plaid <- scgsva(pbmcs, hsko, method = "plaid")

# View results
head(res_plaid@gsva[, 1:5])
```
...
| `kcdf` | Kernel for CDF (GSVA): `"Poisson"` or `"Gaussian"` | `"Poisson"` |
| `mx.diff` | GSVA scoring: max-min (`TRUE`) or max deviation (`FALSE`) | `TRUE` |
| `abs.ranking` | Absolute ranking flag (with `mx.diff=TRUE`) | `FALSE` |
| `ssgsea.norm` | Normalize ssGSEA scores | `TRUE` |
| `min.sz` / `max.sz` | Gene set size filter | `1` / `Inf` |
| `batch` | Chunk size for batch processing | `1000` |
| `cores` | Number of parallel workers | `4` |
| `use.original` | Use original GSVA package (for validation) | `FALSE` |
| `maxRank` | Max genes to rank per cell (UCell only, auto-adjusted) | `1500` |
| `useTerm` | Use pathway names (`TRUE`) or IDs (`FALSE`) | `TRUE` |

### 2.7 Batch Processing for Large Datasets
...
### 2.8 Validating Against Original GSVA Package

The custom C++ implementation produces identical results to the original GSVA
Bioconductor package. You can verify this with `use.original = TRUE`:

```{r validate, eval=FALSE}
# Custom C++ implementation (default, fast)
res_custom <- scgsva(pbmcs, hsko, method = "ssgsea")

# Original GSVA package (requires GSVA installed)
res_orig <- scgsva(pbmcs, hsko, method = "ssgsea", use.original = TRUE)

# Compare: should be identical
cor(res_custom$gsva[,1], res_orig$gsva[,1])  # 1.0
```
...
# Each cluster shows two half-violins, one per group
...
| Function | Description |
|----------|-------------|
| `scgsva()` | Run ssGSEA/GSVA/PLAGE/z-score/UCell analysis |
...
## Python Companion: pygsva

For Python users, the companion package [**pygsva**](https://github.com/guokai8/pygsva) provides the same four enrichment methods (ssGSEA, GSVA, PLAGE, z-score) with NumPy-optimized implementations:

```python
# Python example
from pygsva import gsva, gsvaParam, ssgsea

# GSVA
param = gsvaParam(expr_matrix, gene_sets=gene_sets, kcdf="Gaussian")
result = gsva(param)

# ssGSEA
result = ssgsea(expr_matrix, gene_sets, n_jobs=4)
```

Install with: `pip install pygsva`
